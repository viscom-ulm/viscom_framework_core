...
namespace pro_cal {
	...
	*/
	ProjectorOverlapFramebuffer::ProjectorOverlapFramebuffer(sgct::Engine* engine)
	{
		gEngine = engine;
...
#define GAMMA(x) (pow((1.f/x),(1.f/2.2f)))
	...
	*/
	void ProjectorOverlapFramebuffer::blurTex(GLint blur_texture1, GLint blur_texture2){
		...
		//set uniforms
		glUniform1f(resolution_loc, screenWidth);//screenWidth * 4);
		...
		//setting up uniforms
		glUniform1f(resolution_loc, screenHeight);//screenHeight * 4);
		glUniform1f(radius_loc, blurRadius);
	...
	*/
	bool ProjectorOverlapFramebuffer::CreateContext(const int screenWidth, const int screenHeight,
		...
		numberOfProjectorPoints = projectorPoints.size() / 2; //TODO: test  / 2 => wegen color data
		numberOfProjectors = overlapingVerticesCounter.size();
	...
	*/
	bool ProjectorOverlapFramebuffer::CreateProgram(const std::string& programName,
		const std::string& vertexShaderPath,
		const std::string& fragmentShaderPath)
	{
		if (!sgct::ShaderManager::instance()->shaderProgramExists(programName)){
			sgct::ShaderManager::instance()->addShaderProgram(programName, vertexShaderPath, fragmentShaderPath);
			sgct::ShaderManager::instance()->bindShaderProgram(programName);
			sgct::ShaderManager::instance()->unBindShaderProgram();
		}
	...
	*/
	void ProjectorOverlapFramebuffer::initOGL3BlurTexture(){
		...
		if (!sgct::ShaderManager::instance()->shaderProgramExists("blur_tex")){
			sgct::ShaderManager::instance()->addShaderProgram("blur_tex", SHADER_PATH + "blur_tex.vert", SHADER_PATH + "blur_tex.frag");
			sgct::ShaderManager::instance()->bindShaderProgram("blur_tex");
			sgct::ShaderManager::instance()->unBindShaderProgram();
		}
