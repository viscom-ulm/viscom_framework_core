...
namespace viscom {
    ...
    void SlaveNodeInternal::InitOpenGL()
    {
        ...
        for (auto i = 0U; i < numWindows; ++i) {
            ...
            GetViewportOrigin(i) = glm::ivec2(glm::ceil(((viewport[0] + vpSize) / (2.0f * vpSize)).xy * totalScreenSize));
            GetViewportScaling(i) = totalScreenSize / glm::vec2(1920.0f, 1080.0f);// resolutionScaling;
            GetViewportSize(i) = glm::ivec2(glm::ceil(totalScreenSize));
