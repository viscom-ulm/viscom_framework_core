...
namespace pro_cal {
	...
	*/
	void Slave::init() {
		...
		windowsCount = thisNode->getNumberOfWindows();
	...
	*/
	void Slave::checkQuadCornersFromMaster(const Shared_Msg cmd, const std::vector<cv::Point2f> quadCornersData) {
		if (cmd.msg == SHOW_FINAL && (cmd.slaveID == this->slaveID || cmd.slaveID == ALL) && !quadCornersData.empty() && quad_corners[0].empty()) {
			for (unsigned int window = 0; window < windowsCount; window++) {
				int projectorNo = getProjectorNo(slaveID, window);
				int idx = projectorNo * 4;
				for (unsigned int i = idx; i < (idx + 4); i++) {
	...
	void Slave::checkLocalLowHighVPFromMaster(const Shared_Msg cmd, const std::vector<cv::Point2f> LocalLowHighVP_Data) {
		if (cmd.msg == SHOW_FINAL && (cmd.slaveID == this->slaveID || cmd.slaveID == ALL) && !LocalLowHighVP_Data.empty() && LocalLowHighVP[0].empty()) {
			for (unsigned int window = 0; window < windowsCount; window++) {
				...
				for (unsigned int i = idx; i < (idx + 2); i++) {
...
void Slave::init_final(sgct::Engine * gEngine, const std::vector<int> coords, const int window) {
    ...
    */
	void Slave::show_final(sgct::Engine * gEngine, int window) {
		...
		glViewport(coords[0], coords[1], coords[2] * resolutionScale[window].x, coords[3] * resolutionScale[window].y);
		...
		//TODO: test
		GLfloat withAlphaTrans_loc = sgct::ShaderManager::instance()->getShaderProgram("OpenGL3R2T").getUniformLocation("withAlphaTrans");
		glUniform1i(withAlphaTrans_loc, alphaTransition);

		GLfloat resolution_loc = sgct::ShaderManager::instance()->getShaderProgram("OpenGL3R2T").getUniformLocation("resolution");
		glUniform2f(resolution_loc, coords[2], coords[3]);
    ...
    */
	void Slave::calc_vert_points(int window)
	{
		...
		resolutionScale[i].x = (MaxX - MinX) * 1.1;
		resolutionScale[i].y = (MaxY - MinY) * 1.1;
	}
    ...
    */
	void Slave::draw_scene(int window){
		...
		for (int quad = 0; quad < Slave::numberOfQuads; quad++)
		{
			//first triangle
    ...
    */
	void Slave::initVAO_BackgroundGrid_3v4c(int window){
		...
		float delta = 0.125f;
		for (float x = -6 - 1.778; x < 6; x += delta) {
			float green = (x + 6) / 12;
		...
		int numberOfVertices = backgroundGrid.size();
    ...
    */
	void Slave::initTmpFBO_1T(int window)
	{
		...
		// NewCode
		fb_width = (float)(fb_width)* resolutionScale[window].x;
		fb_height = (float)(fb_height)* resolutionScale[window].y;
