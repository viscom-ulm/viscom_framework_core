...
namespace pro_cal {
	...
	*/
	void Master::loadAndSendDataToSlaves() {
		...
		for (int i = 0; i < projector_count; i++) {		
			readVectorOfPoint2f(DAT_FILE, "quad_corners" + std::to_string(i), quad_corners_Vec[i]);
			readVectorOfPoint3f(DAT_FILE, "TexCoordinates" + std::to_string(i), TexCoordinates_Vec[i]);
			readVectorOfPoint2f(DAT_FILE, "LocalLowHighVP" + std::to_string(i), VPCoordinates1D_Vec[i]);
		...
		readVectorOfPoint2f(DAT_FILE, "ViewPlaneCoordinates", ViewPlaneCoordinates1D);
		readVectorOfPoint2f(DAT_FILE, "projectorCorners", projectorCorners1D);	
		
		std::vector<ColorCalibData> colorCalibData(projector_count);
		loadColorCalibData(colorCalibData);
	...
	*/
	void Master::init() {
		...
		nextStep = 0;
		current_color = 0;
		slave_count = sgct_core::ClusterManager::instance()->getNumberOfNodes();
