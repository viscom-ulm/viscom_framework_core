...
void    ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
    ...
    int fb_width = (int)(ioExt->viewportSize_.x);
    int fb_height = (int)(ioExt->viewportSize_.y);
    ...
    glViewport(0, 0, ioExt->viewportSize_.x, ioExt->viewportSize_.y);

    // Our visible imgui space lies from draw_data->DisplayPps (top left) to draw_data->DisplayPos+data_data->DisplaySize (bottom right). DisplayMin is typically (0,0) for single viewport apps.
    // glViewport(0, 0, (GLsizei)fb_width, (GLsizei)fb_height);
    float L = static_cast<float>(-ioExt->viewportOrigin_.x) / io.DisplayFramebufferScale.x;
    float R = static_cast<float>(-ioExt->viewportOrigin_.x) / io.DisplayFramebufferScale.x + io.DisplaySize.x;
    float T = static_cast<float>(-ioExt->viewportOrigin_.y) / io.DisplayFramebufferScale.y;
    float B = static_cast<float>(-ioExt->viewportOrigin_.y) / io.DisplayFramebufferScale.y + io.DisplaySize.y;
